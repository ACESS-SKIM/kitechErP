...
export default function EditProductMaterial({ closeEvent, currentPartID, editMaterial }) {
  ...
  const [partrecycledcontents, setPartRecycledContents] = useState(() => editMaterial?.recyclingcontent || '');
  ...
  useEffect(() => {
    const fetchPartSerialName = async () => {
      const partDocRef = doc(db, 'productparts', currentPartID);
